...
export default class Home extends Component {
  ...
  render() {
    ...
    const {
      ...
      dailyForecast,
      hourly12Forecast,
    } = this.context.state.local
    // console.log(Object.keys(this.context.state))
    // console.log(cityName)
    return (
      <SafeAreaView style={{ flex: 1 }}>
        <ImageBackground
          source={getIcon(current.icon, current.isDayTime)[1]}
          ...
          <ScrollView contentContainerStyle={styles.container}>
            <View style={styles.header}>
              ...
              <Text style={[styles.headerMaxMin, styles.headerText]}>
                Máx.: {dailyForecast[0].maxTemp}° Mín.:{' '}
                {dailyForecast[0].minTemp}°
              </Text>
              <Image
                source={getIcon(current.icon, current.isDayTime)[0]}
